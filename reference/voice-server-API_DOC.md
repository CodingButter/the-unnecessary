...
### `POST /api/voices` — add a new voice

Upload a reference clip (multipart form) to register a new voice. The clip is
transcoded server-side to **mono 24 kHz MP3** and stored, so it becomes
selectable by `id` in every synthesis request afterward. A clean **5–20 s mono**
sample (minimal noise, no music, one speaker) clones best. Accepts
`wav`/`mp3`/`m4a`/`ogg`/`flac`; max 25 MB.

| Field | Type | Default | What it does |
|-------|------|---------|--------------|
| `name` | string (form) | *required* | Voice name; sanitized to the on-disk `id`. |
| `file` | file (form) | *required* | The reference audio clip. |
| `overwrite` | bool (form) | `false` | Replace an existing voice of the same name (else `409`). |

```bash
# Add a voice from a local file
curl -X POST http://localhost:8080/api/voices -b cookies.txt \
  -F 'name=Jamie' -F 'file=@/path/to/jamie_sample.wav'
# -> {"id":"Jamie","name":"Jamie","format":"mp3"}
```

Returns `409` if the name already exists (retry with `-F overwrite=true`), `400`
for an unsupported/undetectable format, `413` if over 25 MB.

### One-off voice (no save) — `sample_base64`

To read text in a voice **without** saving it, pass a base64 reference clip on
`/api/generate` or `/api/synthesize` instead of `voice`. The sample is used for
that single request and never written to `voices/`.

| Field | Type | Default | What it does |
|-------|------|---------|--------------|
| `sample_base64` | string | none | base64-encoded reference clip. Overrides `voice` when set. |
| `sample_format` | string | `"mp3"` | Source format: `mp3`/`wav`/`m4a`/`ogg`/`flac`. |

```bash
# Read text in a one-off voice straight from a local clip (no save)
SAMPLE=$(base64 -w0 /path/to/clip.wav)   # macOS: base64 -i /path/to/clip.wav | tr -d '\n'
curl -X POST http://localhost:8080/api/generate -b cookies.txt \
  -H 'Content-Type: application/json' \
  -d "{\"sample_base64\":\"$SAMPLE\",\"sample_format\":\"wav\",\"text\":\"Reading in a brand-new voice.\"}" \
  -o out.wav
```
...
Every synthesis request accepts these fields. `text` is always required; supply
**either** `voice` **or** `sample_base64` (see [One-off voice](#one-off-voice-no-save--sample_base64)).

| Field | Type | Range | Default | What it does |
|-------|------|-------|---------|--------------|
| `voice` | string | — | *required\** | Voice `id` from `/api/voices`. (\*omit when using `sample_base64`.) |
| `sample_base64` | string | — | none | base64 reference clip for a one-off voice; overrides `voice`. |
| `sample_format` | string | mp3/wav/m4a/ogg/flac | `"mp3"` | Source format of `sample_base64`. |
...
| `timestamps` | bool | `false` | Also return per-segment **and per-word** start/end times. |
  ...
  "segments": [                  // only present when timestamps=true
    {
      ...
      "words": [                 // per-word timings (Whisper word_timestamps)
        {"word": " The",        "start": 0.0,  "end": 0.30, "probability": 0.99},
        {"word": " full",       "start": 0.30, "end": 0.62, "probability": 0.98},
        {"word": " transcript", "start": 0.62, "end": 1.10, "probability": 0.97}
      ]
    }
  ]
}
...
> Each `word` keeps the leading space Whisper emits on its tokens (rejoin with a
> plain concatenation, not a space-join). `probability` is the model's per-token
> confidence (0–1). `words` is present on every segment when `timestamps=true`.
...
supports `X-Play-Local: true` to play on the server's own speakers, and a
`sample_base64` (+ `sample_format`) arg to clone a one-off voice for a single
utterance; `upload_voice` saves a clip as a permanent named voice (`audio_base64`
or `source_url`); `transcribe` takes audio as `audio_base64` or a `source_url`
and returns the text. This is for MCP clients (e.g. Claude Code) and is gated by
`X-API-Key`; the REST endpoints above are the path for your own scripts.

There's also a **local** stdio MCP server (`@jleelove/voice-mcp`, `npx -y
@jleelove/voice-mcp`) that runs on your machine so it can read local files and
play audio locally — see `voice-mcp/README.md`. It exposes the same four tools,
with `synthesize` taking a local `sample_path` and `upload_voice` taking a local
`path`.
