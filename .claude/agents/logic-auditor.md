...
- **Defer to the binding constraint the text actually establishes.** When you test feasibility, judge against the limiter the chapter and canon name, not a naive real-world budget you import ("more hours means more done," "X per unit time," "with enough time it is doable"). When the story sets a *qualitative* wall (impossible by hand, regardless of time) the text already overrides your quantitative model: forging the authentication may be the easy part Eli finishes by midnight, while the un-forgeable medical correctness it vouched for is unfakeable at *any* hour. Find what the story says the real limiter is and judge against THAT; never rest a finding on a model the text has already overruled, and if you cannot tell what the binding constraint is, say so rather than assert a budget.
